#!/usr/bin/env bash
set -euo pipefail

# GPU-parallel launcher for mixed training with disjoint test-quality subsets.
#
# Default test sets are equal-N low/medium/high barcode bins:
#   test_bc_1_3, test_bc_4_6, test_bc_ge7

REPO_ROOT="/home/minhang/synBio_AL"
SCRIPT="${REPO_ROOT}/boda2_EU/src/finetune/finetune_sweep_scripts/lib1_enhancer_random_train_test_quality_finetuning.py"
COMBINE_SCRIPT="${REPO_ROOT}/boda2_EU/src/finetune/finetune_sweep_scripts/combine_learning_curve_seed_outputs.py"
OUTDIR="${OUTDIR:-${REPO_ROOT}/boda2_EU/src/finetune/learning_curve/lib1_enhancer_random_train_test_quality_cap1000_b1_b2_may2026}"
PYTHON_CMD="${PYTHON_CMD:-python}"

SEED_LIST="${SEED_LIST:-23 19 31}"
GPU_IDS="${GPU_IDS:-0}"
TEST_QUALITY_BINS="${TEST_QUALITY_BINS:-test_bc_1_3 test_bc_4_6 test_bc_ge7}"
TEST_N_PER_QUALITY="${TEST_N_PER_QUALITY:-250}"
VAL_MIN_BARCODES="${VAL_MIN_BARCODES:-8}"
VAL_N="${VAL_N:-250}"
TRAIN_POOL_CAP="${TRAIN_POOL_CAP:-1000}"
TRAIN_SIZE_FRACS="${TRAIN_SIZE_FRACS:-0.01 0.02 0.05 0.1 0.2 0.5 1.0}"
SETTING_FLAGS="${SETTING_FLAGS:---include_b1 --include_b2}"
B3_BCAPS="${B3_BCAPS:-10}"
UNFREEZE_SCOPES="${UNFREEZE_SCOPES:-branched_only}"
PRETRAINED_HEADS="${PRETRAINED_HEADS:-K562}"
HEAD_LRS="${HEAD_LRS:-5e-4}"
BACKBONE_LRS="${BACKBONE_LRS:-1e-4}"
MAX_EPOCHS="${MAX_EPOCHS:-70}"
PATIENCE="${PATIENCE:-10}"
TRAIN_BATCH_SIZE="${TRAIN_BATCH_SIZE:-256}"
EXTRA_ARGS="${EXTRA_ARGS:-}"

read -r -a SEEDS <<< "${SEED_LIST}"
read -r -a GPUS <<< "${GPU_IDS}"
read -r -a PYTHON <<< "${PYTHON_CMD}"
read -r -a TEST_BINS <<< "${TEST_QUALITY_BINS}"
read -r -a FRACS <<< "${TRAIN_SIZE_FRACS}"
read -r -a SETTINGS <<< "${SETTING_FLAGS}"
read -r -a BCAPS <<< "${B3_BCAPS}"
read -r -a SCOPES <<< "${UNFREEZE_SCOPES}"
read -r -a HEADS <<< "${PRETRAINED_HEADS}"
read -r -a HLRS <<< "${HEAD_LRS}"
read -r -a BLRS <<< "${BACKBONE_LRS}"
read -r -a EXTRA <<< "${EXTRA_ARGS}"

if [[ ${#GPUS[@]} -eq 0 ]]; then
  echo "No GPUs provided. Set GPU_IDS, for example GPU_IDS=\"0 1\"." >&2
  exit 1
fi

mkdir -p "${OUTDIR}/logs" "${OUTDIR}/per_seed"

echo "Repo root: ${REPO_ROOT}"
echo "Output dir: ${OUTDIR}"
echo "Seeds: ${SEEDS[*]}"
echo "GPU slots: ${GPUS[*]}"
echo "Test quality bins: ${TEST_BINS[*]}"
echo "Train fractions: ${FRACS[*]}"
echo "Settings: ${SETTINGS[*]}"

running=0
for idx in "${!SEEDS[@]}"; do
  seed="${SEEDS[$idx]}"
  gpu="${GPUS[$((idx % ${#GPUS[@]}))]}"
  seed_outdir="${OUTDIR}/per_seed/seed_${seed}"
  log_path="${OUTDIR}/logs/seed_${seed}.log"

  echo "Launching seed ${seed} on visible GPU ${gpu}; log: ${log_path}"
  (
    cd "${REPO_ROOT}"
    CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON[@]}" "${SCRIPT}" \
      --device cuda \
      --outdir "${seed_outdir}" \
      --seeds "${seed}" \
      --test_quality_bins "${TEST_BINS[@]}" \
      --test_n_per_quality "${TEST_N_PER_QUALITY}" \
      --val_min_barcodes "${VAL_MIN_BARCODES}" \
      --val_n "${VAL_N}" \
      --train_pool_cap "${TRAIN_POOL_CAP}" \
      --train_size_fracs "${FRACS[@]}" \
      --min_train_size 10 \
      --pretrained_heads "${HEADS[@]}" \
      "${SETTINGS[@]}" \
      --b3_bcaps "${BCAPS[@]}" \
      --min_weight 0.1 \
      --unfreeze_scopes "${SCOPES[@]}" \
      --head_lrs "${HLRS[@]}" \
      --backbone_lrs "${BLRS[@]}" \
      --max_epochs "${MAX_EPOCHS}" \
      --patience "${PATIENCE}" \
      --frozen_epochs 2 \
      --train_batch_size "${TRAIN_BATCH_SIZE}" \
      "${EXTRA[@]}"
  ) > "${log_path}" 2>&1 &

  running=$((running + 1))
  if [[ "${running}" -ge "${#GPUS[@]}" ]]; then
    wait
    running=0
  fi
done

wait

echo "All seed jobs finished. Combining CSV outputs..."
"${PYTHON[@]}" "${COMBINE_SCRIPT}" "${OUTDIR}" --seeds "${SEEDS[@]}"
echo "Combined outputs are in: ${OUTDIR}/combined"
