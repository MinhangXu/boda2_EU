#!/usr/bin/env python3
"""Prepare in-house Lib1 FivePrime data for single-output scratch HPO.

The first in-house 5'UTR scratch branch uses exact/modal 50 nt inserts so it is
directly comparable to Hani 5'UTR models that also consume 50 nt sequences.
The script validates DNA sequences, keeps finite positive RNA/DNA targets,
computes log-transformed targets, and writes a learn-ready TSV plus metadata
under `src/learn/derived_data/`.
"""
...
DEFAULT_OUTPUT = (
    ...
    / "utr5"
    / "bashor_in_house"
    / "lib1_fiveprime_modal50_fastqs1_5__learn_ready.tsv"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sequence-column", default="FivePrime")
    ...
    parser.add_argument(
        ...
        default="modal",
        help="Default modal keeps the in-house 50 nt FivePrime branch.",
    )
    parser.add_argument(
        "--exact-length",
        type=int,
...
def main() -> None:
    ...
    if out.empty:
        raise ValueError(
            f"No rows remain after length_policy={args.length_policy} "
