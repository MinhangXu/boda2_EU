...
CONFIG_PATH="configs/utr5/hani_rna_activity/resnet1d/utr5__hani_rna_activity_lib1_lib2__resnet1d__phase3_scratch_bayes.yml"
DATASET_PATH="${LEARN_DIR}/derived_data/utr5/hani_rna_activity/5UTR_lib1_lib2_phase3_branched_observed_heads.csv"
TASK_FAMILY="utr5"
TARGET_FAMILY="hani_rna_activity_lib1_lib2"
COMPARISON_GROUP="utr5__hani_rna_activity__phase3_lib1_lib2_scratch_resnet1d"
LAUNCH_SCRIPT="launch/utr5_hani_lib1_lib2_resnet1d_phase3_scratch_sweep.sh"
WANDB_SWEEP_ENTITY="${WANDB_SWEEP_ENTITY:-}"
WANDB_SWEEP_PROJECT="${WANDB_SWEEP_PROJECT:-}"
TOTAL_RUNS="${TOTAL_RUNS:-64}"

if [[ "${FORCE_REBUILD_DATASET:-0}" == "1" || ! -f "${DATASET_PATH}" ]]; then
  echo "Preparing Phase 3 Lib1+Lib2 dataset at ${DATASET_PATH}"
  (
    cd "${LEARN_DIR}"
    python prepare_hani_utr5_lib1_lib2_phase3_dataset.py
  )
fi
