...
# Wrapper for the Lib1 barcode-threshold downsampling scratch training manifest.
#
# Dry run:
#   cd /home/minhang/synBio_AL/boda2_EU
#   GENERATE_MANIFEST=1 DRY_RUN=1 MAX_ROWS=5 GPU_LIST="0" \
#     bash src/learn/launch/lib1_inhouse_barcode_threshold_downsampling_orchestrator.sh
#
# Full run:
#   cd /home/minhang/synBio_AL/boda2_EU
...
if [[ "${GENERATE_MANIFEST}" == "1" || ! -f "${MANIFEST_JSONL}" ]]; then
  echo "Generating barcode-threshold downsampling manifest: ${MANIFEST_TAG}"
  (
    cd "${LEARN_DIR}"
