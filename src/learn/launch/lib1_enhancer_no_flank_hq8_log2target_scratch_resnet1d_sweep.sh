...
CONFIG_PATH="configs/enhancer/bashor_in_house/resnet1d/lib1_enhancer_no_flank_hq8_log2target__scratch_resnet1d__bayes.yml"
TASK_FAMILY="enhancer"
TARGET_FAMILY="bashor_in_house_lib1_enhancer_no_flank_hq8_fastqs1_5_log2_rna_dna"
COMPARISON_GROUP="enhancer__bashor_in_house__no_flank_hq8_log2target__scratch_resnet1d_fp32"
LAUNCH_SCRIPT="launch/lib1_enhancer_no_flank_hq8_log2target_scratch_resnet1d_sweep.sh"
...
LEARN_READY_DATA="${LEARN_DIR}/derived_data/enhancer/bashor_in_house/lib1_fastqs1_5_0filtered_out__learn_ready_log2_target.tsv"
...
if [[ ! -f "${LEARN_READY_DATA}" && "${DRY_RUN}" != "1" ]]; then
  ...
  echo "Run: python ${LEARN_DIR}/prepare_lib1_enhancer_fastqs1_5_dataset.py --output_path ${LEARN_READY_DATA}" >&2
  exit 1
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "LIB1 ENHANCER NO-FLANK HQ8 LOG2-TARGET SCRATCH RESNET1D HPO"
  echo "CONFIG_PATH:          ${CONFIG_PATH}"
  echo "SOURCE_DATA:          ${SOURCE_DATA}"
  echo "LEARN_READY_DATA:     ${LEARN_READY_DATA}"
  echo "TARGET_COLUMN:        log2_RNA_DNA"
