...
# Wrapper for the Lib1 full n=1 barcode-bin follow-up manifest.
#
# Dry run:
#   cd /home/minhang/synBio_AL/boda2_EU
#   GENERATE_MANIFEST=1 DRY_RUN=1 MAX_ROWS=5 GPU_LIST="0" \
#     bash src/learn/launch/lib1_inhouse_barcode_bin_n1_full_orchestrator.sh
#
# Full run:
#   cd /home/minhang/synBio_AL/boda2_EU
...
MANIFEST_TAG="${MANIFEST_TAG:-lib1_barcode_bin_n1_full_june2026}"
...
if [[ "${GENERATE_MANIFEST}" == "1" || ! -f "${MANIFEST_JSONL}" ]]; then
  echo "Generating full n=1 barcode-bin manifest: ${MANIFEST_TAG}"
  (
    cd "${LEARN_DIR}"
    python generate_lib1_barcode_bin_n1_full_manifest.py --manifest-tag "${MANIFEST_TAG}"
  )
fi
