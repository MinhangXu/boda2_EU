...
CONFIG_PATH="configs/utr5/bashor_in_house/utr_bassetvl/lib1_fiveprime_modal50__scratch_utr_bassetvl__bayes.yml"
TASK_FAMILY="utr5"
TARGET_FAMILY="bashor_in_house_lib1_fiveprime_modal50_fastqs1_5_dedup_exact"
COMPARISON_GROUP="utr5__bashor_in_house__fiveprime_modal50__scratch_utr_bassetvl_fp32"
LAUNCH_SCRIPT="launch/lib1_fiveprime_scratch_utr_bassetvl_sweep.sh"
...
SOURCE_DATA="${SOURCE_DATA:-${LIB1_VARIANT_ROOT}/L1_final_fastqs1-5_sublibrary_FivePrime_subset.dedup_exact.csv}"
LEARN_READY_DATA="${LEARN_DIR}/derived_data/utr5/bashor_in_house/lib1_fiveprime_modal50_fastqs1_5_dedup_exact__learn_ready.tsv"

if [[ "${LENGTH_POLICY}" != "modal" && "${LENGTH_POLICY}" != "exact" ]]; then
  echo "ERROR: ${LAUNCH_SCRIPT} assumes the exact/modal 50 nt FivePrime branch." >&2
  echo "       Use LENGTH_POLICY=modal or LENGTH_POLICY=exact, or create a matching config with a different input_len/padded_seq_len." >&2
  exit 1
fi

if [[ "${PREPARE_DATASET}" == "1" ]]; then
  python "${LEARN_DIR}/prepare_lib1_fiveprime_inhouse_dataset.py" \
...
if [[ ! -f "${LEARN_READY_DATA}" && "${DRY_RUN}" != "1" ]]; then
  ...
  echo "Run: python ${LEARN_DIR}/prepare_lib1_fiveprime_inhouse_dataset.py" >&2
  exit 1
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "LIB1 FIVEPRIME MODAL50 SCRATCH UTR_BASSETVL HPO"
